...
// Get request for home page background color hardcode color
...
// Get request for home page database query for applicable colors
app.get('/home', function(req, res) {
...
// Get request for home/pick_color which is used to update background color from db for home route
...
// post request to allow users to input their own color
...
// Get request for team_stats, query db to fill in table -done
...
app.get('/team_stats', function(req, res) {
  ...
  db.task('get-everything', task => {
	...
	.then(info => {
	  info[0].forEach(function(row){
	  	// console.log(row.visitor_name)
	  	if(row.visitor_score < row.home_score){
	  		winning_team.push('CU Boulder')
	  		cu_wins += 1;
	  	}
	  	else{
	  		winning_team.push(row.visitor_name)
	  		cu_loss += 1;
	  	}
	  });
	  win_loss.push(cu_wins)
	  win_loss.push(cu_loss)
	  res.render('pages/team_stats',{
	  
		my_title: "2018 Season Stats",
		data: info[0],
		win_team: winning_team,
		win_to_loss: win_loss
	  })
...
//   		/player_info - get request (no parameters) - done
//   			This route will handle a single query to the football_players table which will retrieve the id & name for all of the football players.
//   			Next it will pass this result to the player_info view (pages/player_info), which will use the ids & names to populate the select tag for a form 
...
app.get('/player_info/select_player', function(req, res) {
  ...
  var player_man = req.query.player_choice;
  // console.log(req.query.player_choice)
  var player_num_games = 0;
  var player_info =  'select name, year, major, passing_yards, rushing_yards, receiving_yards, img_src' +
  					 ' from football_players where id = ' + player_man +';' ;
  var player_games = 'select * from football_games' +
  					 ' where ' + player_man + ' = ANY (players);';
  var player_id_name =  'select id, name from football_players;';
  ...
  db.task('get-everything', task => {
	...
	.then(info => {
	  ...
	  // });
	  // console.log("Dude!!" + info[1])
	  // console.log("Dude!!" + info[2])
	  info[1].forEach(function(row){
	  	player_num_games = player_num_games + 1;
	  });
